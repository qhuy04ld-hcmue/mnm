...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_post'])) {
        ...
        $delete_sql = "DELETE FROM BaiViet WHERE id = '$id_to_delete'";
        ...
        <div class="articles">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<input type="hidden" name="id_to_delete" value="' . htmlspecialchars($row['id']) . '">';
                    echo '<button type="submit" name="delete_post" class="delete-button">Xóa bài viết</button>';
                    echo '</form>';
                    echo '</div>';
                }
            } else {
